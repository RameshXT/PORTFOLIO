    ...
    stages
    {
        stage("Cleaning the workspace")
        {
            steps
            {
                script
                {
                    def workspaceDir = "/var/lib/jenkins/workspace/portfolio/"
        ...
        stage("Building dockerfile")
        {
            steps
            {
                    script
                    {
                        ...
                        sh "sudo docker build -t ${dockerImageTag} /var/lib/jenkins/workspace/portfolio/"
                    }
                }
        }
        ...
        stage("Running docker images")
        {
            steps
            {
                script
                {
                    ...
                    sh "sudo docker run -i -t -d --name highwaycont -p 80:80 ${dockerImageTag}"
